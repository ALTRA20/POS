...
if ($search) {
    if ($charPertama == '#') {
        ...
        $sql = "SELECT `produk`.*,`jumlah`,`request`,`keranjang`.markup FROM `produk` LEFT JOIN `keranjang` ON `keranjang`.`product_id` = `produk`.`id` AND `keranjang`.`user_id` = '$userId' WHERE `produk`.`id_barcode` LIKE '%$search%' AND `produk`.is_active = 1 ORDER BY `produk`.`created_at` DESC";
    }else{
        $searchFix = '';
        ...
        $sql = "SELECT `produk`.*,`jumlah`,`request`,`keranjang`.markup FROM `produk` LEFT JOIN `keranjang` ON `keranjang`.`product_id` = `produk`.`id` AND `keranjang`.`user_id` = '$userId' WHERE $searchFix AND `produk`.is_active = 1 ORDER BY `produk`.`created_at` DESC";
    }   
}else{
    $sql = "SELECT `produk`.*,`jumlah`,`request`,`keranjang`.markup FROM `produk` LEFT JOIN `keranjang` ON `keranjang`.`product_id` = `produk`.`id` AND `keranjang`.`user_id` = '$userId' WHERE `produk`.is_active = 1 ORDER BY `produk`.`created_at` DESC";
}
