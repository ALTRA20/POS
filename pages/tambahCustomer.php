...
<section class="container py-5">
    <div class="d-flex justify-content-end mb-4">
        <button type="button" class="btn bg-light text-dark" data-bs-toggle="modal" data-bs-target="#tambahCustomer">Tambah Customer</button>
        <div class="modal fade" id="tambahCustomer" tabindex="-1" aria-labelledby="tambahCustomerLabel" aria-hidden="true">
            <div class="modal-dialog">
                <form action="" method="post" class="modal-content bg-dark">
                    ...
                    <div class="modal-body">
                        <div class="">
                            ...
                            <input type="text" id="nama" name="nama" class="form-control text-light mb-3">
                        </div>
                        <div class="">
                            <label for="alamat">alamat</label>
                            <input type="text" id="alamat" name="alamat" class="form-control text-light mb-3">
                        </div>
                        <div class="">
                            <label for="wa">wa</label>
                            <input type="text" id="wa" name="wa" class="form-control text-light mb-3">
                        </div>
                    </div>
    ...
    <?php 
    $costumers = $db->query("SELECT * FROM `customer`");
    foreach ($costumers as $key => $costumer) :
    ?>
        <div class="row <?=($key == 0) ? 'border-top' : ''?> border-bottom py-3">
            <div class="col-5">
                <?=$costumer['nama']?>
            </div>
            <div class="col-3">
                ...
                <div class="modal fade" id="edit<?=$costumer['id']?>" tabindex="-1" aria-labelledby="edit<?=$costumer['id']?>Label" aria-hidden="true">
                    <div class="modal-dialog">
                        <form action="" method="post" class="modal-content bg-dark">
                            ...
                            <div class="modal-body">
                                <div class="">
                                    <input type="text" id="id" name="id" class="form-control d-none text-light mb-3" value="<?=$costumer['id']?>">
                                    <label for="nama">nama</label>
                                    <input type="text" id="nama" name="nama" class="form-control text-light mb-3" value="<?=$costumer['nama']?>">
                                </div>
                                <div class="">
                                    <label for="alamat">alamat</label>
                                    <input type="text" id="alamat" name="alamat" class="form-control text-light mb-3" value="<?=$costumer['alamat']?>">
                                </div>
                                <div class="">
                                    <label for="wa">wa</label>
                                    <input type="text" id="wa" name="wa" class="form-control text-light mb-3" value="<?=$costumer['wa']?>">
                                </div>
                            ...
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                                <button type="submit" class="btn btn-primary" name="edit-customer">Save changes</button>
                            </div>
                        </form>
                    </div>
                </div>
