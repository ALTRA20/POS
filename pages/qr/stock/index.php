...
<div class="bg-dark h-vh-93 overflow-hidden d-flex px-3">
    <div class="bg-dark p-4" id="cart" style="width:35%;">
        <h5 class="">Tambah Stock</h5>
        <div class="overflow-auto" id="cartMain" style="height: 83%;">
            <div class="d-flex align-items-center gap-3 pt-3 d-none" id="cart-name">
        ...
        <div class="d-flex align-items-center justify-content-end d-none" id="btn-lanjut" style="height: 10%;">
...
<script>
    ...
    function toPesanan(userId) {
        let konfirmasi = confirm("Apakah yakin ingin menambah stock barang ini?");
        if (konfirmasi) {
            datas = {
                userId : userId
            }
            fetch('commitHistory.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            .then(response => response.text())
            .then(datas => {
                window.location.href = '/pages/qr/stock/history/';
            })
            .catch((error) => {
                console.error('Error:', error);
            });
    ...
    function submit(id, userId, quantity = 1, qr) {
            ...
            datas.forEach(data => {
                ...
                cards += `<div class="pb-3 border-bottom mb-3" id="produk${data['product_id']}">
                    <div class="d-flex gap-2 mb-2">
                        <div class="w-fit pointer d-flex align-items-center animasi p-2" onclick="hapusDariKeranjang(${data['id']},<?=$userId?>)">
                        ...
                        <div class="w-70 d-flex flex-column justify-content-center position-relative">
                            <div class="m-0">${data['nama']}</div>
                            ...
                            <div class="w-100 d-flex border">
                                ...
                                <div class="w-60 p-0">
                                    <input autocomplete="off" type="text" class="form-control text-light border-0 outline-0 text-center" name="qty" id="valueJumlahKeranjang${data['product_id']}" value="${quantity}" readonly>
                                </div>
            ...
            let search = document.querySelector("#search").value;
                if (search.trim() !== "") {
                    if (!isNaN(search) || search[0] == "#") {
                        submit(search.replace("#", ""),<?=$userId?>,1,true);
                        document.querySelector("#search").value = '';
                    }
                }
        }
    ...
    function sendData(userId,page,search) {
        ...
        fetch('/components/index/qrProducts.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        .then(datas => {
            if(datas['datas'].length == 0){
                let buttonCus = `<div class="d-flex flex-column align-items-center">
                  <h1 class="text-danger d-flex align-items-center fw-bold" style="font-size:100px; font-weight:1200;">PRODUK TIDAK ADA</h1>
                  <div class="rounded-circle p-1 bg-light text-dark w-fit h-fit border border-dark shadow-lg pointer" data-bs-toggle="modal" data-bs-target="#addCustomBarang">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 d-none d-md-block">
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" class="form-control border border-dark text-dark mb-4" id="idBarcodeEdit${id}" value="${idBarcode}" autofocus autocomplete="off" <?=($jabatan != 'super-admin') ? 'readonly' : '' ?> onclick="this.select()">
                                </div>
                                <div class="mb-4">
...
<script>
    ...
    function submitFormEdit(idProduk) {
        ...
        .then(response => response.json())
        .then(datas => {
            window.location.href = "/pages/qr/?ikan=" + nama;
        })
        .catch((error) => {
            console.error('Error:', error);
        });
