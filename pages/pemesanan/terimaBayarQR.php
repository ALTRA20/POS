    ...
    <div class="row">
        <div class="col-md-8 h-100">
            <div class="my-5 alert alert-primary p-2">
                <div class="row alert alert-dark m-0 p-4">
                    ...
                    <div class="col-md-7">
                        <p class="m-0 d-none">Note : <?=$pesanan['note']?></p>
                        <p class="m-0">Tgl Bayar:</p>
                        <input type="date" id="tanggalBayar" class="w-fit form-control border border-dark bg-light" value="<?=date('Y-m-d')?>" onChange="cekButtonKirim()" name="tanggalBayar" disabled>
                    </div>
        ...
        <div class="col-md-4 h-100 py-5">
            <form action="/components/pemesanan/qr.php" method="post" class="w-100 h-100 border border-dark text-dark px-3" style="width:360px;">
                <div class="row my-3">
                    ...
                    <div class="col-8">
                        <select class="form-select border border-dark" id="jalurBayar" aria-label="Default select example"  onChange="cekButtonKirim()" name="bayar">
                            <option value="">--- Jalur Bayar ---</option>
                            <option value="Cash" selected>Cash</option>
                            <option value="BCA">BCA</option>
                            <option value="Lainnya">Lainnya</option>
                        </select>
                        <input type="text" class="d-none" name="qrProduk">
                    </div>
                </div>
                <input type="date" id="tanggalBayar" class="w-fit d-none form-control border border-dark bg-light" value="<?=date('Y-m-d')?>" onChange="cekButtonKirim()" name="tanggalBayar">
                <div class="row my-3">
                    ...
                    <div class="col-8">
                        ...
                        <input type="number" id="bc" class="d-none form-control bg-light border border-dark" name="bc" value="1">
                    </div>
                </div>
                <div class="row my-3 text-success fw-bold" style="font-size:25px;">
                    <div class="col-4 d-flex justify-content-end">Kembalian:</div>
                    <div class="col-8" id="kembalian">0</div>
                    <div class="col-8 d-none" id="kurangBayar"><?=$total?></div>
                </div>
                ...
                <input type="text" class="d-none" name="idPesanan" value="<?=$id_pesanan?>">
                <input type="text" class="d-none" name="nominalHarusDibayar" value="<?=$total?>">
...
<script>
    ...
    document.querySelector("#bayar").addEventListener("input", function () {
        let kurang = this.value - document.querySelector("#totalHarga").innerHTML.replace(/Rp/g, "").replace(/\./g, "");
        if (!this.value || this.value == 0 || kurang == 0) {
            kurang = 0;
        }else{
            kurang = rupiah(kurang);
        }
        document.querySelector("#kembalian").innerHTML = kurang;
    })
