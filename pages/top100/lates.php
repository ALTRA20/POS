...
<div class="container py-5">
  <h1 class="text-dark mb-5">Top 100 Lates</h1>
  ...
  $totals = $db->query("SELECT produk.*, 
    produk_id, 
    SUM(CAST(jumlah AS UNSIGNED)) AS total_terjual,
    MAX(pesanan.id) AS last_order_id
  FROM pesanan_detail
  ...
  AND produk.is_tampil_top100 = $s
  AND pesanan.created_at >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)
  GROUP BY produk_id
  ...
  <div class="<?=$kelaso;?> d-flex justify-content-between align-items-center">
    ...
    <div class="d-flex align-items-center gap-2">
      <a href="/pages/assets/produk.php?id=<?=$id?>" target="_blank" class="btn btn-warning">add foto</a>
